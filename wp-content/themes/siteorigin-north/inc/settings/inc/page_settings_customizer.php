	...
	public function customize_register( $wp_customize ) {
		...
		if ( method_exists( $wp_customize, 'add_panel' ) ) {
			$wp_customize->add_panel( 'page_settings', array(
				'title' => __( 'Page Template Settings', 'siteorigin-north' ),
				'description' => __( 'Change layouts for various pages on your site.', 'siteorigin-north' ),
				'priority' => 11,
			) );
		}
		...
		$types = array();
		$templates = array(
			'home' => __( 'Blog Page', 'siteorigin-north' ),
			'search' => __( 'Search Results', 'siteorigin-north' ),
			'date' => __( 'Date Archives', 'siteorigin-north' ),
			'author' => __( 'Author Archives', 'siteorigin-north' ),
			'404' => __( 'Not Found', 'siteorigin-north' ),
		);
		...
		foreach ( $post_types as $post_type => $post_type_data ) {
			...
			$types[] = array(
				'group' => 'archive',
				'id' => $post_type,
				'title' => __( 'Type', 'siteorigin-north' ) . ': ' . $post_type_data->label,
			);
		}
		...
		foreach ( $taxonomies as $tax_slug => $taxonomy ) {
			...
			$types[] = array(
				'group' => 'taxonomy',
				'id' => $tax_slug,
				'title' => __( 'Taxonomy', 'siteorigin-north' ) . ': ' . $taxonomy->label,
			);
		}
