...
class SiteOrigin_Settings_About_Page {
	...
	public function about_page_notice() {
		...
		<div class="updated notice is-dismissible">
			<p>
				<?php echo esc_html( sprintf( __( 'Thanks for choosing %s!', 'siteorigin-north' ), $theme->get( 'Name' ) ) ); ?>
				<?php
				printf(
			esc_html__( 'You can learn more about it %1$shere%2$s, or head straight to the %3$scustomizer%4$s to start setting it up.', 'siteorigin-north' ),
			...
			<p>
				<a href="<?php echo esc_url( admin_url( 'themes.php?page=siteorigin-theme-about' ) ); ?>" class="button-primary">
					<?php echo esc_html( sprintf( __( 'Learn About %s', 'siteorigin-north' ), $theme->get( 'Name' ) ) ); ?>
				</a>
			</p>
		</div>
		<?php
	...
	public function add_theme_page() {
		...
		add_theme_page(
			sprintf( __( 'About %s', 'siteorigin-north' ), $theme_name ),
			sprintf( __( 'About %s', 'siteorigin-north' ), $theme_name ),
			'edit_theme_options',
	...
	public function enqueue_scripts( $prefix ) {
		if ( $prefix !== 'appearance_page_siteorigin-theme-about' ) {
			return;
		}
	...
	public function get_share_link( $network ) {
		$theme = wp_get_theme( get_template() );
		$share_url = false;
			...
			case 'twitter':
				$share_url = add_query_arg( array(
					'status' => urlencode(
						$theme->get( 'Name' ) .
						' - ' .
						__( 'Free #WordPress Theme from @SiteOrigin', 'siteorigin-north' ) . ' - ' .
						$theme->get( 'ThemeURI' )
					),
	...
	public function display_about_page() {
		...
		$about = apply_filters( 'siteorigin_about_page', array(
			'title' => sprintf( __( 'About %s', 'siteorigin-north' ), $theme->get( 'Name' ) ),
			'sections' => array(),
			'title_image' => false,
		...
		<div class="wrap" id="siteorigin-about-page">
				...
				<?php if ( ! empty( $about['tour_url'] ) ) { ?>
					<li>
						<a href="<?php echo esc_url( $about['tour_url'] ); ?>" class="about-button-tour" target="_blank">
							<?php esc_html_e( 'Take a Tour', 'siteorigin-north' ); ?>
						</a>
					</li>
				<?php } ?>
				...
				<?php if ( ! empty( $about['newsletter_url'] ) ) { ?>
					<li>
						<a href="<?php echo esc_url( $about['newsletter_url'] ); ?>" class="about-button-updates" target="_blank">
							...
							<?php esc_html_e( 'Get Updates', 'siteorigin-north' ); ?>
						</a>
					</li>
				<?php } ?>
				...
				<?php if ( ! empty( $about['documentation_url'] ) ) { ?>
					<li>
						<a href="<?php echo esc_url( $about['documentation_url'] ); ?>" class="about-button-docs" target="_blank">
							...
							<?php esc_html_e( 'Documentation', 'siteorigin-north' ); ?>
						</a>
					</li>
				<?php } ?>
				...
				<?php if ( ! empty( $about['review_url'] ) ) { ?>
					<li>
						<a href="<?php echo esc_url( $about['review_url'] ); ?>" class="about-button-updates" target="_blank">
							...
							<?php esc_html_e( 'Write a Review', 'siteorigin-north' ); ?>
						</a>
					</li>
				<?php } ?>
				...
				<?php if ( ! empty( $about['premium_url'] ) && ! class_exists( 'SiteOrigin_Premium' ) ) { ?>
					<li class="about-highlight">
						<a href="<?php echo esc_url( $about['premium_url'] ); ?>" class="about-button-updates" target="_blank">
							...
							<?php esc_html_e( 'Upgrade to Premium', 'siteorigin-north' ); ?>
						</a>
					</li>
				<?php } ?>
						...
						<?php if ( empty( $about['no_video'] ) ) { ?>
							<div class="about-video-watch">
								<a href="<?php echo esc_url( $about['video_url'] ); ?>" target="_blank">
									<?php esc_html_e( 'Watch The Video', 'siteorigin-north' ); ?>
								</a>
							</div>
						<?php } ?>
						...
						<?php if ( $theme->get( 'ThemeURI' ) ) { ?>
							<div class="about-share">
								<div class="about-share-title">
									<?php echo esc_html( sprintf( __( 'If you like %s, please share it!', 'siteorigin-north' ), $theme->get( 'Name' ) ) ); ?>
								</div>
			...
			<div class="about-siteorigin-logo">
				<p>
					<?php echo esc_html( __( 'Proudly Created By', 'siteorigin-north' ) ); ?>
				</p>
